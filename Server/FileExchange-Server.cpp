...
int main(int argc, char *argv[]){
    ...
    RSA *rsa = NULL;
    const char *rsaPrivKeyFilename = "ServerPrivateKey.pem", *fileOk = "OK", *fileErr = "ERROR";
    ...
    if(!bFile){
        lenEncBuf = RSA_private_encrypt(strlen(fileErr)+1, (unsigned char*)fileErr, (unsigned char*)encBuf, rsa, RSA_PKCS1_PADDING);
        SSL_write(ssl, encBuf, lenEncBuf);                      // If file doesn't exist, send error message to the client
        free(encBuf);
        cout << "  Error 404: File not found!" << endl << endl;
    }else{
        lenEncBuf = RSA_private_encrypt(strlen(fileOk)+1, (unsigned char*)fileOk, (unsigned char*)encBuf, rsa, RSA_PKCS1_PADDING);
        SSL_write(ssl, encBuf, lenEncBuf);                      // Send OK to the client before transmitting whole file
